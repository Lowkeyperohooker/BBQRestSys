    ...
    <div class="flex-1 p-6 md:p-8 overflow-y-auto flex flex-col bg-gray-50/30">
      ...
      <div v-else-if="viewMode === 'new'" class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        <div v-for="item in availableItems" :key="item.prep_item_id" @click="addToCart(item)"
          ...
          <span v-if="item.is_variable_price"
            class="absolute top-3 right-3 bg-orange-50 text-orange-600 text-[10px] font-bold px-2.5 py-1 rounded-full z-10">
            Variable
          </span>

          <div
            class="h-20 bg-blue-50/50 rounded-xl mb-4 flex items-center justify-center text-blue-500 group-hover:scale-105 transition-transform duration-300">
          ...
          <h4 class="font-bold text-gray-900 truncate mb-1" :title="item.pos_display_name">{{ item.pos_display_name }}
          </h4>
          <div class="flex justify-between items-end mt-auto pt-2">
            <div class="flex flex-col">
              <span v-if="item.is_variable_price"
                class="text-[10px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Starts at</span>
      ...
      <div v-else-if="viewMode === 'active'" class="flex-1">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <div v-for="order in activeOrders" :key="order.order_id"
            class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col relative overflow-hidden">
            <div :class="order.status === 'Cooking' ? 'bg-orange-400' : 'bg-green-400'"
            ...
            <div class="mt-auto flex flex-col gap-3 border-t border-gray-50 pt-4">
              ...
              <div class="flex gap-2 w-full mt-2">
                <BaseButton v-if="order.status === 'Cooking'" variant="secondary"
                  @click="handleUpdateStatus(order, 'Cooked')" class="flex-1 py-2 text-sm">
                  Mark Cooked
                </BaseButton>
    ...
    <div v-show="viewMode === 'new'" class="w-80 bg-white border-l border-gray-100 flex flex-col h-full shrink-0">
      ...
      <div class="p-6 border-b border-gray-50">
        <h3 class="text-xl font-extrabold text-gray-900 mb-4">
          Order Details
          <span v-if="totalCartItems > 0" class="text-sm font-medium text-blue-500 ml-2">({{ totalCartItems }}
            items)</span>
        </h3>
        ...
        <div class="flex bg-gray-100 rounded-xl p-1 w-full">
          <button @click="orderType = 'Dine-in'"
            :class="orderType === 'Dine-in' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'"
            class="flex-1 py-2 text-sm font-semibold rounded-lg transition-all">Dine-in</button>
          <button @click="orderType = 'Takeout'"
            :class="orderType === 'Takeout' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'"
            class="flex-1 py-2 text-sm font-semibold rounded-lg transition-all">Takeout</button>
        </div>
      </div>
        ...
        <div v-else class="space-y-5">
          <div v-for="(cartItem, index) in cart" :key="index" class="flex justify-between items-center group">
            <div class="flex-1">
              ...
              <p class="text-sm font-medium text-gray-500">PHP {{ cartItem.unit_price.toFixed(2) }} x {{ cartItem.qty }}
              </p>
            </div>
            <div class="flex items-center gap-4">
      ...
      <div class="p-6 bg-white border-t border-gray-100">
        ...
        <BaseButton variant="primary" @click="handleSendToGrill" :disabled="cart.length === 0 || !customerName.trim()"
