...
async function checkActiveTabs() {
  // Only poll if the user has access to POS
...
<template>
  ...
  >
    ...
    <div v-if="currentUser" class="px-6 py-4 bg-gray-800/40 border-b border-gray-800 flex flex-col">
      <span :class="['font-bold text-gray-200', fontBase]">{{ currentUser.name }}</span>
      <span :class="['text-blue-400 font-medium mt-0.5', fontSm]">{{ currentUser.role }}</span>
    </div>
    ...
    <nav :class="['flex-1 px-4 py-4 space-y-1 overflow-y-auto pb-24 md:pb-6', fontBase]">
      ...
      <router-link v-if="hasAccess(['Admin'])" to="/inventory" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-gray-800 hover:text-white" active-class="bg-blue-600 text-white shadow-md">
      ...
      <router-link v-if="hasAccess(['Staff'])" to="/prep" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-gray-800 hover:text-white" active-class="bg-blue-600 text-white shadow-md">
      ...
      <router-link v-if="hasAccess(['Admin', 'Staff'])" to="/schedule" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-gray-800 hover:text-white" active-class="bg-blue-600 text-white shadow-md">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
        Staff Scheduling
      </router-link>
      
      <router-link v-if="hasAccess(['Admin'])" to="/staff" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-gray-800 hover:text-white" active-class="bg-blue-600 text-white shadow-md">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
        Staff Directory
      </router-link>

      <router-link v-if="hasAccess(['Admin'])" to="/logs" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-gray-800 hover:text-white" active-class="bg-blue-600 text-white shadow-md">
    ...
    <div class="p-4 border-t border-gray-800 mt-auto">
      <button 
        @click="handleLogout" 
        :class="['w-full flex items-center justify-center gap-2 px-4 py-2.5 text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded-lg transition-colors', fontBase]"
      >
