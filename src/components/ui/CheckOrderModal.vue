...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-100 flex items-end justify-end p-4 md:p-8 bg-black/60 backdrop-blur-md" @click.self="$emit('close')">
    <div class="bg-surface-container-low border border-outline-variant/20 w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden animate-in slide-in-from-bottom-8 duration-300 origin-bottom-right">
      ...
      <div class="flex-1 overflow-y-auto p-5">
        <div class="space-y-4">
          <div v-for="(cartItem, index) in cart" :key="index" class="flex justify-between items-center group bg-surface-container py-3 px-4 rounded-xl border border-outline-variant/5">
            <div class="flex-1 pr-2">
              <p :class="['font-bold text-on-surface', fontBase]">{{ cartItem.pos_display_name }}</p>
      ...
      <div class="p-6 bg-surface-container-highest/20 border-t border-outline-variant/10 shrink-0">
        <div :class="['flex justify-between mb-3 text-on-surface-variant font-medium', fontSm]">
          <span>Subtotal</span><span>₱{{ subtotal.toFixed(2) }}</span>
        </div>
        <div :class="['flex justify-between mb-6 font-black text-on-surface', font2xl]">
