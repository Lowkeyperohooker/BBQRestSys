  ...
  try {
    if (newItemType.value === 'raw') {
      ...
      if (selectedFile.value) {
        // Upload the physical file first, then get the URL back
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="flex-1 overflow-y-auto p-6 bg-white">
        ...
        <form v-else-if="internalTab === 'new_item'" @submit.prevent="handleAddNewItem" class="space-y-5">
          <div class="flex gap-2 p-1 bg-gray-100 rounded-lg">
            <button type="button" @click="newItemType = 'raw'" :class="[newItemType === 'raw' ? 'bg-white shadow text-gray-900 font-bold' : 'text-gray-500', 'flex-1 py-2 text-sm rounded-md transition-all']">Raw Meat</button>
            <button type="button" @click="newItemType = 'prepared'" :class="[newItemType === 'prepared' ? 'bg-white shadow text-gray-900 font-bold' : 'text-gray-500', 'flex-1 py-2 text-sm rounded-md transition-all']">POS Item</button>
          </div>

          <template v-if="newItemType === 'raw'">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1.5">Category (e.g. Pork)</label>
          ...
          <template v-else>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1.5">Menu Tab</label>
            ...
            <div>
              ...
              >
                ...
                <div v-if="previewUrl" class="relative w-full">
                  ...
                  <button type="button" @click.stop="removeFile" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1.5 hover:bg-red-600 shadow-md">
                ...
                <div v-else class="text-center">
                  ...
                  <p class="mt-2 text-sm text-gray-500 font-medium">Drag & drop or click to browse</p>
                </div>
              </div>
            </div>
