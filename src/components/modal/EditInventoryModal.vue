  ...
  try {
    if (newItemType.value === 'raw') {
      ...
      await inventoryService.addPreparedItem(
        ...
        finalPhotoUrl,
        null, null // Standard item, no variants typed manually
      );
    }
      ...
      <div class="flex-1 overflow-y-auto p-6 bg-surface">
        <form v-if="internalTab === 'adjust'" @submit.prevent="handleAdjustStock" class="space-y-5">
          ...
          <div>
            ...
            <select v-model="adjustItemId" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors">
          ...
          <div class="flex gap-4">
            <div class="flex-1">
              ...
              <input v-model.number="adjustAmount" type="number" step="0.1" required placeholder="e.g. 5 or -2" class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-bold focus:border-primary-container outline-none transition-colors" />
            </div>
            <div class="flex-1">
              <label class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Reason</label>
              <select v-model="adjustReason" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors">
          ...
          <BaseButton type="submit" variant="primary" class="w-full py-3 mt-2">Save Adjustment</BaseButton>
        </form>
        ...
        <form v-else-if="internalTab === 'new_item'" @submit.prevent="handleAddNewItem" class="space-y-5">
          ...
          <template v-if="newItemType === 'raw'">
            <div>
              ...
              <input v-model="newRawCategory" type="text" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
            </div>
            <div>
              <label class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Specific Cut</label>
              <input v-model="newRawPart" type="text" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
            </div>
            <div>
              <label class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Alert Threshold (kg)</label>
              <input v-model.number="newRawAlert" type="number" step="0.1" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
            </div>
          </template>
          ...
          <template v-else>
            <div>
              ...
              <select v-model="newPrepCategory" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors">
            ...
            <div>
              ...
              <input v-model="newPrepName" type="text" required placeholder="e.g. Bottled Coke" class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
            </div>

            <div>
              <label class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Base Price (PHP)</label>
              <input v-model.number="newPrepPrice" type="number" step="0.01" min="0" required class="w-full bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
            </div>

            <div>
              <label class="block text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-1.5">Item Photo (Optional)</label>
              <div @click="!previewUrl ? fileInput?.click() : null" :class="['w-full border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center transition-colors', !previewUrl ? 'cursor-pointer border-outline-variant/30 hover:bg-surface-container min-h-32' : 'border-transparent']">
                ...
                <div v-else class="text-center">
                  <p class="text-sm text-on-surface-variant font-medium">Click to upload photo</p>
                </div>
              </div>
            </div>
          </template>

          <BaseButton type="submit" variant="primary" class="w-full py-3 mt-2">Add Item</BaseButton>
        </form>
        ...
        <div v-else class="space-y-6">
          ...
          <form @submit.prevent="handleAddCategory" class="pt-4 border-t border-outline-variant/20">
            ...
            <div class="flex gap-2">
              <input v-model="newCategoryName" type="text" required placeholder="e.g. Beverages" class="flex-1 bg-surface-container text-on-surface border border-outline-variant/30 rounded-xl px-4 py-3 font-medium focus:border-primary-container outline-none transition-colors" />
              <BaseButton type="submit" variant="success" class="px-6 py-2">Add</BaseButton>
            </div>
          </form>
        </div>
